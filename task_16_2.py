"""
Task 2

Mathematician

Implement a class Mathematician which is a helper class for doing math
operations on lists

The class doesn't take any attributes and only has methods:

square_nums (takes a list of integers and returns the list of squares)
remove_positives (takes a list of integers and returns it without positive
numbers
filter_leaps (takes a list of dates (integers) and removes those that are
not 'leap years'
"""


class Mathematician:

    def square_nums(self, m_list):
        square_list = []
        for item in m_list:
            square_list.append(item ** 2)
        print(square_list)

    def remove_positives(self, m_list):
        remov_posit = []
        for item in m_list:
            if item < 0:
                remov_posit.append(item)
        print(remov_posit)

    def filter_leaps(self, m_list):
        not_leap_years = []
        for item in m_list:
            if item % 4 == 0:
                not_leap_years.append(item)
        print(not_leap_years)


m = Mathematician()

m.square_nums([7, 11, 5, 4])  # == [49, 121, 25, 16]

m.remove_positives([26, -11, -8, 13, -90])  # == [-11, -8, -90]

m.filter_leaps([2001, 1884, 1995, 2003, 2020])  # == [1884, 2020]
